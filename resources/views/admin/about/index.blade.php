   ...
   @include('layouts.admin.meassage')
        ...
        <div class="panel">
          ...
          <header class="panel-heading">
            <h3 class="panel-title float-">Add New About</h3>
            
          </header>
          
          <div class="panel-body">
          <a href="{{route('admin.about.create')}}" class="btn btn-primary float-right">Add About</a>
            <table class="table table-hover dataTable table-striped w-full" id="exampleTableTools">
             ...
             @foreach ($abouts as $key => $about)
               <tr role="row" class="odd">
                ...
                <td>{{\Carbon\Carbon::parse($about->updated_at)->diffForHumans()}}</td>
                  <td>
            
                  <a href="{{route('admin.about.edit',$about->about_id)}}" class="btn btn-success btn-sm" >Edit</a>
                 <button type="button" class="btn btn-danger btn-sm" onclick="deleteCategory({{ $about->about_id }})">

                                            <i class="material-icons">delete</i>

                                        </button>

                                        <form id="delete-form-{{$about->about_id}}" action="{{route('admin.about.destroy',$about->about_id)}}" method="POST" style="display:none">
                                            @csrf
                                            @method('DELETE')
                                        </form>
...
<script type="text/javascript">
        ...
        function deleteCategory(about_id) {
