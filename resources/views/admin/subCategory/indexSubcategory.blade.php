...
@section('content')
   ...
   @include('layouts.admin.meassage')
        ...
        <div class="panel">
          ...
          <a href="{{route('admin.subcategory.create')}}" class="btn btn-primary float-right">Add SubCategory</a>
            <table class="table table-hover dataTable table-striped w-full" id="exampleTableTools">
             ...
             @foreach ($subcategories as $key => $subcategory)
               <tr role="row" class="odd">
                  <td class="sorting_1" tabindex="0">{{$subcategory->subCatagoryID}}</td>
                  <td>{{$subcategory->subCatagoryName}}</td>
                  <td>{{$subcategory->catagory->catagoryName}}</td>
                  <td>{{\Carbon\Carbon::parse($subcategory->created_at)->diffForHumans()}}</td>
                <td>{{ \Carbon\Carbon::parse($subcategory->updated_at)->diffForHumans()}}</td>
                  <td>
                  <a href="{{route('admin.subcategory.edit',$subcategory->subCatagoryID)}}" class="btn btn-success btn-sm" >Edit</a>
                 <button type="button" class="btn btn-danger btn-sm" onclick="deleteCategory({{ $subcategory->subCatagoryID }})">

                                            <i class="material-icons">delete</i>

                                        </button>

                                        <form id="delete-form-{{$subcategory->subCatagoryID}}" action="{{route('admin.subcategory.destroy',$subcategory->subCatagoryID)}}" method="POST" style="display:none">
                                            @csrf
                                            @method('DELETE')
                                        </form>
...
<script type="text/javascript">
        ...
        function deleteCategory(subCatagoryID) {
            ...
            swalWithBootstrapButtons({
                ...
                document.getElementById('delete-form-'+subCatagoryID).submit();
            } else if (
