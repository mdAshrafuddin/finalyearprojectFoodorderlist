...
@section('content')
   ...
   @include('layouts.admin.meassage')
        ...
        <div class="panel">
          ...
          <a href="{{route('admin.fooditem.create')}}" class="btn btn-primary float-right">Add SubCategory</a>
            <table class="table table-hover dataTable table-striped w-full" id="exampleTableTools">
             ...
             @foreach ($fooditems as $key => $fooditem)
               <tr role="row" class="odd">
                  <td class="sorting_1" tabindex="0">{{$fooditem->foodItem_id}}</td>
                  <td>{{$fooditem->food_name}}</td>
                  <td>{{$fooditem->food_description}}</td>
                  <td>{{$fooditem->price}}</td>
                  <td>{{$fooditem->food_tag}}</td>
                  <td>{{$fooditem->subcatagory->subCatagoryName}}</td>
                  <td>{{\Carbon\Carbon::parse($fooditem->created_at)->diffForHumans()}}</td>
                <td>{{\Carbon\Carbon::parse($fooditem->updated_at)->diffForHumans()}}</td>
                  <td>
                  <a href="{{route('admin.fooditem.edit',$fooditem->foodItem_id)}}" class="btn btn-success btn-sm" >Edit</a>
                 <button type="button" class="btn btn-danger btn-sm" onclick="deleteCategory({{ $fooditem->foodItem_id }})">

                                            <i class="material-icons">delete</i>

                                        </button>

                                        <form id="delete-form-{{$fooditem->foodItem_id}}" action="{{route('admin.fooditem.destroy',$fooditem->foodItem_id)}}" method="POST" style="display:none">
                                            @csrf
                                            @method('DELETE')
                                        </form>
...
<script type="text/javascript">
        ...
        function deleteCategory(fooditem) {
            ...
            swalWithBootstrapButtons({
                ...
                document.getElementById('delete-form-'+fooditem).submit();
            } else if (
