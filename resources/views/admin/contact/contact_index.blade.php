   ...
   @include('layouts.admin.meassage')
        ...
        <div class="panel">
          ...
          <header class="panel-heading">
            <h3 class="panel-title float-">Add New Contact</h3>
            
          </header>
          
          <div class="panel-body">
          <a href="{{route('admin.contact.create')}}" class="btn btn-primary float-right">Add Category</a>
            <table class="table table-hover dataTable table-striped w-full" id="exampleTableTools">
             ...
             @foreach ($contacts as $key => $contact)
               <tr role="row" class="odd">
                  ...
                  <td>{{$contact->coonact_name}}</td>
                  <td>{{$contact->email}}</td>
                  <td>{{$contact->mobile_number}}</td>
                  <td>{{$contact->meassage}}</td>
                  <td>{{\Carbon\Carbon::parse($contact->created_at)->diffForHumans()}}</td>
                <td>{{\Carbon\Carbon::parse($contact->updated_at)->diffForHumans()}}</td>
                  <td>
                  <a href="{{route('admin.contact.edit',$contact->contact_id)}}" class="btn btn-success btn-sm" >Edit</a>
                 <button type="button" class="btn btn-danger btn-sm" onclick="deleteCategory({{ $contact->contact_id }})">

                                            <i class="material-icons">delete</i>

                                        </button>

                                        <form id="delete-form-{{$contact->catagoryID}}" action="{{route('admin.contact.destroy',$contact->contact_id)}}" method="POST" style="display:none">
                                            @csrf
                                            @method('DELETE')
                                        </form>
